...
parser.add_argument(
    "--save_dir", default="", type=str, help="path to accuracy records"
)
parser.add_argument(
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # Data loading code: training with mnist, testing with svhn
    normalize = transforms.Normalize(
        mean=[0.7440, 0.5895, 0.7213], std = [0.1787, 0.2131, 0.1721]
    )

    
    dataset = get_dataset(dataset="camelyon17", download=False)
    train_dataset = dataset.get_subset('train',transform = transforms.Compose(
            [
    ...
    val_dataset = dataset.get_subset('test',transform = transforms.Compose(
            [
                transforms.ToTensor(),
                normalize,
            ]
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if robust_dataset is None:
            ...
            train_rb(train_loader, robust_loader, model, criterion, optimizer, epoch, args)
        ...
        if epoch == 0:
            acc = acc1.unsqueeze(dim=0).cpu().numpy()
            print(acc)
            print(acc.shape)
        else:
            acc = np.concatenate([acc, acc1.unsqueeze(dim=0).cpu().numpy()])

        
    np.savetxt(args.save_dir, acc, fmt="%f", delimiter=',', header='acc')
...
def train_rb(train_loader, robust_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target, _) in enumerate(train_loader):
        try:
            images2, target2, _ = next(robust_loader_iterator)
